'use client';
// SplashCursor — a WebGL fluid simulation that "splashes" gold dye where the
// cursor moves. Desktop/fine-pointer only, and disabled under reduced-motion.
// Self-contained (raw WebGL2/WebGL, no extra deps). Adapted from the well-known
// fluid-cursor effect, tinted to the GSTify Ivory-Gold palette.
...
export default function SplashCursor({
  ...
  useEffect(() => {
    // Only run on fine-pointer (mouse) desktop devices, and respect reduced motion.
    ...
    // Narrow once here so the non-null type carries into every closure below.
    ...
    const config: any = {
      ...
      BACK_COLOR: { r: 0, g: 0, b: 0 }, TRANSPARENT,
    };
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      if (!glCtx) return { gl: null as any, ext: {} as any };
      let halfFloat: any;
      ...
      if (isWebGL2) {
        ...
        halfFloat = (glCtx as any).getExtension('OES_texture_half_float');
        supportLinearFiltering = (glCtx as any).getExtension('OES_texture_half_float_linear');
      }
    ...
    class Material {
      ...
      programs: any[] = []; activeProgram: WebGLProgram | null = null; uniforms: any = {};
    ...
    // Gold-tinted dye to match the Ivory-Gold theme.
    function generateColor() {
      const palette = [
        { r: 0.66, g: 0.49, b: 0.18 }, // #A87E1E gold
        { r: 0.84, g: 0.69, b: 0.37 }, // #D7B05F champagne
        { r: 0.73, g: 0.54, b: 0.18 }, // #B98A2E
        { r: 0.78, g: 0.60, b: 0.21 }, // warm gold
      ];
    ...
    let rafId = requestAnimationFrame(updateFrame);
